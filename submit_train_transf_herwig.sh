#!/bin/bash
#PBS -q gpu
#PBS -N ParticleTransformer-kinpid
#PBS -l walltime=72:00:00
#PBS -l mem=8gb
#PBS -l ncpus=4
#PBS -l ngpus=1
#PBS -o output-transformer_herwig_kinpid.log
#PBS -e error-transformer_herwig_kinpid.log
...
python train.py \
    ...
    --model-prefix training/QuarkGluon_Tranformers_herwig/${model}/net \
    ...
    --start-lr $lr --optimizer ranger --log logs/QuarkGluon_Tranformers_herwig_${model}.log --predict-output pred.root \
    --tensorboard QuarkGluon_Tranformers_herwig_${FEATURE_TYPE}_${model} \
    ${extraopts} "${@:3}"
