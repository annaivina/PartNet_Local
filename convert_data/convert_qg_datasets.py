...
def _transform(X, y, start=0, stop=-1):
    ...
    v['part_pid'] = PID
    v['part_isCHPlus'] =  ((PID == 211) + (PID == 321) + (PID == 2212) + (PID == -3222) + (PID == 3222) + (PID == -3312) ).astype(np.float32)
    v['part_isCHMinus'] = ((PID == -211) + (PID == -321) + (PID == -2212) + (PID == 3312) + (PID == -3112) + (PID == 3112) + (PID == 3334) ).astype(np.float32)
    v['part_isNeutralHadron'] = ((PID == 130) + (PID == 2112) + (PID == -2112) + (PID == 3322) + (PID == -3322) + (PID == -3122) + (PID == 3122) + (PID == 310) ).astype(np.float32)
...
if __name__ == '__main__':
    ...
    import glob
    sources = natural_sort(glob.glob(os.path.join(args.inputdir, '*.npz')))
